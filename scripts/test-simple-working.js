const { SimpleSPHINCSKeyGen, SimpleSPHINCSSignature, SimpleSPHINCSVerifier } = require('../lib/sphincs/simple-working');
const fs = require('fs');

async function main() {
  console.log("Testing Simple Working SPHINCS+ Implementation...\n");

  try {
    // Generate key pair
    console.log("1. Generating key pair...");
    const keyGen = new SimpleSPHINCSKeyGen(32, 8, 4, 16);
    ...
    const signer = new SimpleSPHINCSSignature(keyPair.privateKey);
    ...
    const verifier = new SimpleSPHINCSVerifier(keyPair.publicKey);
    ...
    // Generate key pairs for smart contract testing
    ...
    contractKeyPairs.forEach((kp, index) => {
      fs.writeFileSync(
        `${keysDir}/simple_keypair_${index + 1}.json`,
        JSON.stringify(kp, null, 2)
      );
    });

    fs.writeFileSync(
      `${keysDir}/simple_test_results.json`, 
      JSON.stringify(testResults, null, 2)
    );

    console.log("\nSimple SPHINCS+ testing completed successfully!");
    console.log("Test results saved to: ./test-keys/simple_test_results.json");
    ...
    console.log(`   Performance: ${successCount === numTests ? 'Excellent' : 'Good'}`);
